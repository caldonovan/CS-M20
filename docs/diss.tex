...
The Boolean Satisfiability Problem is one of the most important and renowned in computer science. Forming the basis of NP-Complete problems, it has always been an area of particular interest to computer scientists for a plethora of reasons.

SAT was the first problem to be proved NP-Complete by both Steven Cook and Leonid Levin in 1971 and 1973 respectively\cite{scook}\cite{levin}. It was later included in a well-known paper \italic{Reducibility Among Combinatorial Problems} by Richard Karp\cite{karp}, which generated a renewed interest amongst researchers.

As defined by Donald Knuth, the Boolean Satisfiability Problem is defined as such:

% TODO: Add citation for this quote.
\begin{quotation}
...
Given this definition, we can present a formula as defined above:

\begin{center}
    $F(x1,x2,x3) = (x_1 \vee \bar{x_2}) \wedge (x_2 \vee x_3) \wedge (\bar{x_1} \vee \bar{x_3}) \wedge (\bar{x_1} \vee \bar{x_2} \vee x_3)$
\end{center}

This formula can be satisfied when $x_1x_2x_3 = 001$.

We can consider this CNF formula as a set of clauses, where each clause contains a set of literals. We can use the symbol \wedge to denote an \italic{empty} clause, a clause which contains no literals and in turn is unsatisfiable. A clause in which there is only a single literal is known as a \italic{unit} clause. The 3-SAT problem is considered NP-Complete, and is typically considered the "show child" of NP-Complete SAT.
