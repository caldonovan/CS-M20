...
\subsection{SAT From Other Problems}
\lipsum[2-3]

\subsection{Complete vs. Incomplete}
When we talk about SAT solvers, they can be sorted into two categories; complete and incomplete. Complete solvers are
...
%\end{itemize}
Modern SAT solvers typically fall into two categories: \textit{Conflict Driven Clause Learning} and local search based. Out of the two, CDCL solvers are the most commonly used. Local-search based solvers are usually not used as they are often not considered \textit{complete}. Alongside this, they are usually slower than modern CDCL solvers. Outside of these two reasons, there are situations where local-search can be used on specific problems. % TODO: Expand

\subsubsection{CDCL Solvers}
\textit{Conflict Driven Clause Learning} solvers are themselves an evolution of the original \textit{Davis-Putnam-Logemann-Loveland} algorithm. A CDCL solver is normally both complete and sound. The difference between a traditional DPLL solver and a CDCL solver is the inclusion of \textit{non-chronological backtracking}, or \textit{backjumping}. This feature allows the solver to determine at what point a variable assignment was made, and jump back multiple nodes to avoid running into the same conflict multple times.

The way in which we can implement this \textit{backjumping} is by analysing the current conflict, often called \textit{conflict clause}. This finds the earliest variable assignment that has lead to the conflict and jumps back to it. This conflict clause is then added to the problem to revisiting nodes within the search space that have already been visited.
...
This solver capitalises on modern techniques of using appropriate solvers for sections of a problem. By combining multiple solving algorithms
...
\subsection{Algorithm D - Satisfiability by Cyclic DPLL}
...
This algorithm uses both unit

\subsection{Algorithm L - Cyclic DPLL with Lookahead}
This algorithms extends our initial version of Algorithm D by adding look-ahead procedures.

This algorithm will form the basis of our program and each step will be translated to appropriate code. This algorithm relies on the 'active ring' data structure that Knuth mentions previous to the algorithm. This active ring is key in the operations of the algorithm as we need to have a method of easily deleting and restoring variables in clauses. For C++, it is convienient for us to use the \texttt{std::vector} type when creating the data structure.
...
will explain how the cyclic DPLL algorithm presented by Donald Knuth will do this.
